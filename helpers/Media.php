...
class Media
{
    ...
    static public function img_redimensiona($img = null, $pasta = null, $quality = 100)
    {
        ...
        if (strlen($pasta) > 0 && strlen($img) > 0) {
            ...
            ob_start();
            return $resizeObj->saveImage($path, $quality);
        } else {
            return 0;
        }
    }

    static public function watermark($img, $url_img, $pasta)
    {
        $dir = Path::base();
        ...
        $path = $dir . $ds . 'media' . $ds . $pasta . $ds . $url_img;

        $typeImage = strtolower(substr($img, strlen($img) - 3));

        // if ($typeImage == 'png') {
        //     Media::Tojpg($path, $path, 100);
        // }
        $opacity = 0.3;
    ...
    static public function file_upload($file, $pasta = '', $newname = null)
    {
        ...
        if (!empty($nome) && !empty($arquivo_tmp)) {
            ...
            if (strstr('.jpg;.jpeg;.png;.gif;.pdf;.doc;.docx;.xls;.xlx;.txt;.ppt;.pptx;.zip;.rar;.xml', $extensao)) {
                // cria novo nome para a img
                $nome_arquivo = explode(".", $nome);
                $nome_arquivo[0] = str_replace('.', '-', $nome_arquivo[0]);
                if ($newname == null) {
    ...
    // VIDEO THUMBNAIL
    static public function get_video_thumbnail($link)
    {
    ...
    static function Tojpg($originalFile, $outputFile, $quality)
    {
